...
// DD
template <class T_data> inline int Preprocessor<T_data>::GetClausesSize() {
  // return ins->clauses.size();

  std::cout << "PREP GET CLAUSES RETURN 10" << std::endl;

  if (ins == nullptr) {
    std::cout << "PREP INSTANCE IS NULL" << std::endl;
  } else {
    std::cout << "PRPE INS IS OK????" << std::endl;
  }
