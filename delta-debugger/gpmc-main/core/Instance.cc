...
template <class T_data> void Instance<T_data>::printVarMapStats() const {
...
template <class T_data> void Instance<T_data>::writeVarMap(std::ostream &out) {
  using namespace std;

  // c p vmap orignal_vars simplified_vars
  out << "p vmap " << gmap.size() << " " << npvars << " " << fixedLits.size()
      << " " << freeLitClasses.size() << " " << definedVars.size() << endl;
  ...
  if (projected) {
    for (int i = 0; i < gmap.size(); i++) {
      if (gmap[i] != lit_Undef)
        out << "v " << (pvars[i] + 1) << " " << (sign(gmap[i]) ? "-" : "")
            << (var(gmap[i]) + 1) << endl;
    }
  } else {
    for (int i = 0; i < gmap.size(); i++)
      if (gmap[i] != lit_Undef)
        out << "v " << (i + 1) << " " << (sign(gmap[i]) ? "-" : "")
            << (var(gmap[i]) + 1) << endl;
    ;
  }
}
...
void Instance<T_data>::importVarScore(std::istream &in) {
  ...
  while (getline(in, buf)) {
    ...
    if (tokens.size() == 0)
      continue;
    if (tokens.size() != 2) {
      cerr << "c c Parse Error! Invalid line in the given varscore file!"
           << endl;
      exit(1);
    } else {
...
// For Debug
template <class T_data> void Instance<T_data>::toDimacs(std::ostream &out) {
  using namespace std;
  ...
  if (weighted) {
    ...
    for (int i = 0; i < npvars; i++) {
      out << "c p weight " << (i + 1) << " " << lit_weights[toInt(mkLit(i))]
          << " 0" << endl;
      out << "c p weight -" << (i + 1) << " " << lit_weights[toInt(~mkLit(i))]
          << " 0" << endl;
    }
